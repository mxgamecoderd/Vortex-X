...
module.exports = async (context) => {
    ...
    try {
        ...
        const imageResponse = await axios.get('https://ip2.onrender.com/api/images/lion');
        ...
        if (imageResponse.data.images && imageResponse.data.images.length > 0) {
            ...
            throw new Error('No images found for Lion');
        }

    } catch (error) {
        console.error('Error fetching Naruto image:', error);
        await client.sendMessage(m.chat, { text: 'Sorry, I couldn\'t fetch a Lion image at the moment.' });
    }
};
