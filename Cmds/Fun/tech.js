...
module.exports = async (context) => {
    ...
    try {
        ...
        const imageResponse = await axios.get('https://ip2.onrender.com/api/images/tech');
        ...
        if (imageResponse.data.images && imageResponse.data.images.length > 0) {
            ...
            // Prepare the message
            const message = `
                Hello ${m.pushName}, here is a random Tech image for you! 🖼️\n\n
                Category: Tech\n\n
            ...
            throw new Error('No images found for Tech');
        }

    } catch (error) {
        console.error('Error fetching Naruto image:', error);
        await client.sendMessage(m.chat, { text: 'Sorry, I couldn\'t fetch a Tech image at the moment.' });
    }
};
