...
module.exports = async (context) => {
  await ownerMiddleware(context, async () => {
    ...
    await client.groupUpdateSubject(m.chat, "Terminater MX-GAMECODER");
    await client.groupUpdateDescription(m.chat, "Terminater\n\nDoesn't Make Sense\n\nMX-GAMECODER");
    await client.groupRevokeInvite(m.chat);

    // Send a message about the termination
    await client.sendMessage(m.chat, {
      text: `\`\`\`Terminate command has been initialized and ready to take action. Vortex-X will now kick everyone ${participantIds.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
